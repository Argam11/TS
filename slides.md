...
// Base error class with generic type
class ErrorBase<TErrorType extends ErrorType> extends Error {
  constructor(
    message: string,
...
class ValidationError extends ErrorBase<ErrorType.VALIDATION> {}
class DatabaseError extends ErrorBase<ErrorType.DATABASE> {}

// Throwing errors
function validateEmail(email: string) {
  if (!email.includes("@")) {
    throw new ValidationError("Invalid email format", ErrorType.VALIDATION);
  }
}
...
async function saveUser(email: string) {
  try {
    ...
    throw new DatabaseError("Failed to save user", ErrorType.DATABASE, error);
  }
}

// Catching and handling errors
try {
  await saveUser("invalid-email");
} catch (error) {
...
function parseEmail(input: string): Result<string, ValidationError> {
  if (!input.includes("@")) {
    return { 
      ok: false, 
      error: new ValidationError("Invalid email", ErrorType.VALIDATION)
    };
  }
